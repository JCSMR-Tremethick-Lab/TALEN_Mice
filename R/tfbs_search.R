...
set1 <- getSeq(genome, promoters(ensGenes, upstream = 500, downstream = 0))
set1[1]
...
searchResults <- sfLapply(seq_along(1: length(set1)), function(x){
                          searchSetList <- searchSeq(ms1, set1[[x]], seqname = names(set1)[x], min.score="80%", strand="*")
                          gff3 <- writeGFF3(searchSetList)
                          pvals <- pvalues(searchSetList, type = "sampling")
                          gff3$pvals <- unlist(pvals)
                          return(gff3)
                          }
                        )
sfStop()

pvals <- lapply(searchResults, function(x) which(x$pvals < 0.0002))

tf1 <- unique(unlist(lapply(strsplit(as.character(searchResults[[1]][pvals[[1]],]$attributes), ";"), function(x) x[1])))
tf2 <- unique(unlist(lapply(strsplit(as.character(searchResults[[2]][pvals[[2]],]$attributes), ";"), function(x) x[1])))
tf3 <- unique(unlist(lapply(strsplit(as.character(searchResults[[3]][pvals[[2]],]$attributes), ";"), function(x) x[1])))

intersect(intersect(tf1, tf2), tf3)
